...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    if (isNaN(mainTarget) || isNaN(menuTarget) || isNaN(scaleTarget)) return;
    ...
    Animated.spring(mainBorder, { toValue: borderTarget, damping: 22, stiffness: 160, useNativeDriver: false }).start();
  ...
  return (
    <ErrorBoundary>
      ...
      <Animated.View style={[st.menuContainer, {
        width: safeWidth,
        ...
        {SideMenuComp ? <SideMenuComp onClose={handleCloseMenu} /> : (
          <View style={st.menuLoading}><ActivityIndicator color="#7C3AED" size="large" /></View>
        )}
      </Animated.View>

      <Animated.View style={[st.mainScreen, {
      ...
      {menuVisible && (
        <Pressable style={[st.overlay, isRTL ? { right: safeWidth * 0.72, left: 0 } : { left: safeWidth * 0.72, right: 0 }]} onPress={handleCloseMenu} />
      )}
    </ErrorBoundary>
  );
}
