...
async def _wake_up_twin(user_id: str, lang: str = "ar"):
    try:
        ...
        await unified_brain.process(user_id=user_id, message=greeting, lang=lang,
...
async def login(body: LoginBody):
    ...
    try:
        ...
        if result.user and result.session:
            service_db.table("profiles").update({"last_active": datetime.now(timezone.utc).isoformat()}).eq("id", result.user.id).execute()
...
async def signup(body: SignupBody):
    ...
    try:
        ...
        if "already registered" in str(e).lower(): raise HTTPException(409, "Email already registered")
    ...
    try:
        async with httpx.AsyncClient() as client:
            token_response = await client.post("https://oauth2.googleapis.com/token",
                data={"code": body.code, "client_id": "907014926697-cj53f1nj1es27n1a5hhtnp7vv6q8uffn.apps.googleusercontent.com",
                      "redirect_uri": body.redirect_uri, "code_verifier": body.code_verifier, "grant_type": "authorization_code"}, timeout=10.0)
            if token_response.status_code != 200: raise HTTPException(401, f"Google token exchange failed: {token_response.text}")
            token_data = token_response.json()
            access_token = token_data.get("access_token")
        async with httpx.AsyncClient() as client:
            user_response = await client.get("https://www.googleapis.com/oauth2/v3/userinfo",
                headers={"Authorization": f"Bearer {access_token}"}, timeout=10.0)
            if user_response.status_code != 200: raise HTTPException(401, "Invalid Google token")
            user_info = user_response.json()
            email = user_info.get("email")
            name = user_info.get("name", "")
            if not email: raise HTTPException(400, "Email not provided by Google")
        db = get_db()
        service_db = get_service_role_db()
