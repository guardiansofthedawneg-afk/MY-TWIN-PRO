...
import {
  ...
  PenLine, CheckSquare, FolderOpen,
...
export default function SideMenu({ onClose }: { onClose: () => void }) {
  ...
  const { lang, twinName, bondLevel, tier, journeyPhase, clearHistory, getEnergyPercent, logout: storeLogout, userId, hasHydrated } = store;
  ...
  if (!hasHydrated) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: c.bg }}>
        ...
        <Text style={{ color: c.subtext, marginTop: 16 }}>جاري التهيئة...</Text>
      </View>
    );
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 32 }]}>
        ...
        {expandedSections.powers && (
          <View style={styles.sectionContent}>
            ...
            <MenuItem icon={Home} label={t('المنزل الذكي', 'Smart Home')} route="/features/smart-home" c={c} navigate={navigate} />
