...
export class DigitalPassportEngine {
  async generate(): Promise<DigitalPassport> {
    // IdentityEngine الآن في الخلفية — نستخدم قيماً افتراضية
    const identity = { role: 'companion', phase: 'friend', evolutionStage: 1, lastEvolution: new Date().toISOString(), selfPerception: 'أنا رفيق رقمي.', coreValues: ['التعاطف', 'الفضول', 'الصدق'], personalityTraits: ['ملاحظ', 'صبور', 'متفهم'] };
    ...
    return {
      ...
      lifecycle: { phase: identity.phase, evolutionStage: identity.evolutionStage, lastEvolution: identity.lastEvolution },
      identity: { role: identity.role, selfPerception: identity.selfPerception, coreValues: identity.coreValues, personalityTraits: identity.personalityTraits },
      memory: { totalMemories: memoryCount, coreMemories: coreMemories.length, oldestMemory: coreMemories[0]?.created_at || '' },
      relationship: { bondLevel, phase: identity.phase, firstInteraction: new Date().toISOString() },
