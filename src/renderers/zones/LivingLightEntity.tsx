...
const generateMembranePath = (phase: number, scale: number, points: number, focusLevel: number, emotionIntensity: number) => {
  ...
  for (let i = 0; i < points; i++) {
    ...
    const distortion = Math.sin(angle * 4 + phase) * (12 + emotionIntensity * 8) + Math.cos(angle * 6 + phase * 0.7) * (8 + focusLevel * 6) + focusLevel * Math.sin(angle * 2) * 5;
...
export default function LivingLightEntity({ isThinking = false, isSpeaking = false, isListening = false, onLongPress, onPress }: LivingLightEntityProps) {
  ...
  useEffect(() => {
    const unsubscribe = stateBus.on('presence:state_updated', (_: string, data: any) => {
      ...
      if (newGaze === 'user') { eyeGazeX.value = withTiming(0, { duration: 400 }); eyeGazeY.value = withTiming(-2, { duration: 400 }); }
      else if (newGaze === 'memory') { eyeGazeX.value = withTiming(-8, { duration: 400 }); eyeGazeY.value = withTiming(-4, { duration: 400 }); }
      else if (newGaze === 'internal') { eyeGazeX.value = withTiming(0, { duration: 400 }); eyeGazeY.value = withTiming(6, { duration: 400 }); }
      else { eyeGazeX.value = withTiming(4, { duration: 800 }); eyeGazeY.value = withTiming(-1, { duration: 800 }); }

      if (data.isThinking) setHeadTilt(-3);
      else if (data.microExpressions?.some((m: any) => m.type === 'core_tilt')) {
        setHeadTilt(2);
        setTimeout(() => setHeadTilt(0), 1500);
        audioMixer.playEffect('comfort'); // صوت إمالة الرأس
      } else setHeadTilt(0);

      membranePhase.value = withTiming(membranePhase.value + 0.5, { duration: 2000 });

      if (data.memoryEchoIntensity > 0.5) audioMixer.playMemoryEcho();
      if (data.emotion !== currentEmotion) audioMixer.setEmotionAudio(data.emotion);
    });

    // الاستماع لمستوى الصوت من المستشعرات
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { blink(); scheduleNext(); }, 2000);
  ...
  // الجسيمات الحية (120 جسيم) – تتأثر بمستوى الصوت
  const particles = useRef(
    Array.from({ length: 120 }, (_, i) => ({
      ...
      orbitType: Math.random() > 0.6 ? 'orbit' as const : Math.random() > 0.5 ? 'attract' as const : 'escape' as const,
      colorShift: Math.random() > 0.5,
    }))
  ).current;

  const membranePath = useDerivedValue(() => generateMembranePath(membranePhase.value, 1.0, 60, focusLevel.value, Math.abs(warmth.value - 0.5) * 2), [membranePhase, focusLevel, warmth]);
  const coreRadius = useDerivedValue(() => 14 + breathPhase.value * 10 + energyLevel.value * 4, [breathPhase, energyLevel]);
  const leftEyePath = useDerivedValue(() => generateEyePath(CX - 22, CY - 8, 18 + eyeScale.value * 3, Math.max(1, 10 + eyeScale.value * 2 + (isBlinking ? -8 : 0)), eyeGazeX.value, eyeGazeY.value), [eyeScale, eyeGazeX, eyeGazeY, isBlinking]);
  const rightEyePath = useDerivedValue(() => generateEyePath(CX + 22, CY - 8, 18 + eyeScale.value * 3, Math.max(1, 10 + eyeScale.value * 2 + (isBlinking ? -8 : 0)), eyeGazeX.value, eyeGazeY.value), [eyeScale, eyeGazeX, eyeGazeY, isBlinking]);
  const eyeGlowRadius = useDerivedValue(() => 10 + breathPhase.value * 4, [breathPhase]);

  return (
    <Pressable onPress={onPress} onLongPress={onLongPress} style={[styles.container, { transform: [{ rotate: `${headTilt}deg` }] }]}>
      <Canvas style={{ width: ENTITY_SIZE, height: ENTITY_SIZE }}>
        <Group>
          {/* الطبقة 1: الضباب المحيط */}
          <Circle cx={CX} cy={CY} r={ENTITY_SIZE * 0.5} opacity={0.04 + energyLevel.value * 0.05 + silenceLevel.value * -0.02}>
            <Paint><BlurMask blur={60} style="normal" /></Paint>
            <RadialGradient c={vec(CX, CY)} r={ENTITY_SIZE * 0.5} colors={[emotionColor + '25', 'transparent']} />
          </Circle>

          {/* الطبقة 2: الغشاء الحي */}
          ...
          {[0, 1, 2, 3, 4].map(i => {
            ...
            return (<Circle key={i} cx={x} cy={y} r={18 + energyLevel.value * 14} opacity={0.06 + focusLevel.value * 0.14}><Paint><BlurMask blur={20} style="normal" /></Paint><RadialGradient c={vec(x, y)} r={22} colors={[emotionColor + '35', 'transparent']} /></Circle>);
          })}

          {/* الطبقة 4: الجسيمات الحية – تتأثر بمستوى الصوت */}
          {particles.map(p => {
            const lifeSin = Math.sin(Date.now() / 3000 + p.lifePhase);
            const currentRadius = p.orbitType === 'orbit' ? p.radius : p.orbitType === 'attract' ? p.radius * (0.7 + lifeSin * 0.3) : p.radius * (1.3 + lifeSin * 0.3);
            ...
            const particleColor = p.colorShift ? (warmth.value > 0.6 ? '#FFD700' : emotionColor) : emotionColor;
            return (<Circle key={p.id} cx={px} cy={py} r={p.size * (0.8 + energyLevel.value * 0.4 + audioLevel.value * 2)} color={particleColor} opacity={p.opacity * (0.6 + energyLevel.value * 0.5 + audioLevel.value * 0.5)}><Paint><BlurMask blur={2.5} style="solid" /></Paint></Circle>);
          })}

          {/* الطبقة 5: صدى الذاكرة */}
          {memoryEchoIntensity.value > 0.01 && (<Circle cx={CX} cy={CY} r={ENTITY_SIZE * 0.22 * memoryEchoIntensity.value} opacity={memoryEchoIntensity.value * 0.35}><Paint style="stroke" strokeWidth={1.5} /><BlurMask blur={10} style="normal" /><RadialGradient c={vec(CX, CY)} r={ENTITY_SIZE * 0.22} colors={['#FFFFFF40', '#FFFFFF10', 'transparent']} /></Circle>)}

          {/* الطبقة 6: حقل النية */}
          {intentIntensity.value > 0.01 && (<Circle cx={CX} cy={CY} r={ENTITY_SIZE * 0.28} opacity={intentIntensity.value * 0.3}><Paint style="stroke" strokeWidth={1.8} /><BlurMask blur={12} style="normal" /><SweepGradient c={vec(CX, CY)} colors={[emotionColor + '40', '#FFFFFF30', emotionColor + '40']} /></Circle>)}

          {/* الطبقة 7: نواة البلازما */}
          <Circle cx={CX} cy={CY} r={coreRadius} color={emotionColor} opacity={0.8 * warmth.value}><Paint><BlurMask blur={10} style="solid" /></Paint></Circle>
          <Circle cx={CX} cy={CY} r={4 + breathPhase.value * 3} color="#FFFFFF" opacity={0.6}><Paint><BlurMask blur={3} style="solid" /></Paint></Circle>

          {/* الطبقة 8: العينان */}
          <Circle cx={CX - 22} cy={CY - 8} r={eyeGlowRadius} color={emotionColor} opacity={0.15}><Paint><BlurMask blur={8} style="normal" /></Paint></Circle>
          <Circle cx={CX + 22} cy={CY - 8} r={eyeGlowRadius} color={emotionColor} opacity={0.15}><Paint><BlurMask blur={8} style="normal" /></Paint></Circle>
          <Path path={leftEyePath} color={emotionColor} opacity={0.9} style="fill"><Paint><BlurMask blur={1.5} style="solid" /></Paint></Path>
          <Path path={rightEyePath} color={emotionColor} opacity={0.9} style="fill"><Paint><BlurMask blur={1.5} style="solid" /></Paint></Path>
        </Group>
      </Canvas>
    </Pressable>
  );
